...
if (
  ...
  if (userMessage.includes('скільки контактів') || userMessage.includes('контактів потрібно')) {
    await bot.sendMessage(id, `📊 Кількість контактів для покупки залежить від вашої стратегії та бюджету. Ми радимо звернутися напряму до комерційного директора Vidzone для консультації: a.ilyenko@vidzone.com`);
  ...
  if (userMessage.includes('ott-платформи') || userMessage.includes('на яких платформах') || userMessage.includes('ott')) {
    await bot.sendMessage(id, `📺 Vidzone співпрацює з різноманітними OTT-платформами, лідерами ринку. Деталі ви можете дізнатися, звернувшись до комерційного директора Vidzone: a.ilyenko@vidzone.com.`);

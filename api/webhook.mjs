...
function needsClientStyleGuard(text = '') {
  return /(умовн|наприклад|бренд|об.?єм|обсяг|варіант|таблиц|медіаплан|бриф|пакет|частот|показ)/i.test(String(text || '').toLowerCase());
}
...
- "docs"     — явно просить шаблони документів, гарантійний лист, музична довідка, технічні вимоги до ролика (НЕ плутати з питаннями про умови/правила розміщення реклами — це "rag")
...
export default async function handler(req, res) {
  ...
  // Якщо питання про конкретний бренд + медіаплан/бриф/стратегія — одразу до Анни
  if (/(медіаплан|бриф|стратегі|підбор|рекомендац|план\s+кампан|скільки\s+треба|порахуй|розрахуй)/i.test(rawText)) {
    const reply = 'Для розрахунку кампанії під ваш бренд краще звернутись напряму — ' + CONTACT_ANI + '. Вона підбере пакети, ЦА і бюджет під ваші задачі.';
    ...
    return res.status(200).send('escalate');
  }

  // Дуже нечіткий follow-up без контексту
...
НЕ використовуй markdown: зірочки **, решітки ##. Пиши звичайним текстом.`;
  ...
  try {
    ...
    const ragPrompt = `Ти — AI-помічник Vidzone, компанії з Digital TV реклами в Україні. Спілкуйся як досвідчений колега з медіаринку — по-діловому, але без зайвого офіціозу.

ПРАВИЛА:
- Відповідай українською, стисло і по суті.
- Використовуй інформацію з KB нижче як основу. Якщо питання логічно пов'язане з Digital TV — відповідай впевнено навіть без прямої цитати.
- НЕ вигадуй конкретні цифри яких немає в KB (ціни, відсотки знижок). Загальну логіку медіапланування — можна.
- Якщо відповіді немає і це не про рекламу — скажи: З цього краще уточнити у ${CONTACT_ANI}.
- НЕ використовуй markdown: зірочки **, решітки ##, підкреслення __. Тільки звичайний текст.
- Не згадуй базу знань, KB, RAG, назви файлів.
- Бриф, медіаплан, підбір пакету під бренд — це корисно, роби впевнено.
- Не використовуй фразу якщо хочеш.

ХУКИ — додавай один в кінці відповіді, вибирай ситуативно:
- Можу порахувати [щось конкретне] — скажіть обсяг або ціль.
- Хочете — підберу пакет під вашу ЦА.
- Можу зібрати короткий бриф під це.
- Якщо треба — дам два варіанти: бренд і промо.
- Скажіть категорію — підкажу з яких пакетів стартувати.
- Можу дати орієнтир по бюджету — назвіть ціль кампанії.
- Хочете — поясню різницю між форматами.
- Можу показати як це виглядає для вашої категорії.

ФОРМАТ:
- Ціна/CPM → цифра + що входить + контакт
- Пакети/канали → назви + приклади каналів
- Бриф → задача, ЦА, пакет, частота, період, KPI
- Кейси → бренд + результат у цифрах

ПРИКЛАДИ:
П: Скільки коштує реклама?
В: Базова ціна — 150 грн за 1000 показів (ролик 15 сек). Знижки: пакетні до -15%, бюджетні до -20%. Можу порахувати орієнтовний бюджет під ваш обсяг — скажіть ціль або бюджет.

П: Яка аудиторія?
В: Охоплення Vidzone — 5,9 млн пристроїв на місяць (березень 2026, тренд зростаючий). 81% — вік 18–54, 85% — міста 50K+, 93% — Smart TV. Хочете — покажу які сегменти є під конкретну ЦА.

П: Підбери пакет для CeraVe
В: Для CeraVe — Жіночий пакет, сегменти Леді, Мами, Домогосподарки. Частота 2–3 контакти на тиждень, 3–4 тижні. Якщо потрібне ширше охоплення — додаємо Унісекс. Якщо треба — дам два варіанти: бренд і промо.

П: Чому для фарми це ефективно?
В: На Vidzone SOV фарми лише 14% проти 46% на ТБ — менший клаттер, більше уваги. У 1 кв. 2026 категорія зросла на 68%. Кейс Solgar: +52% продажів лише на Digital TV. Можу підібрати пакет під вашу категорію — скажіть що просуваєте.

П: Скільки показів треба для нормальної кампанії?
В: Орієнтир — від 1,3 млн показів на тиждень для широкої ЦА. Масові кампанії частіше планують 1,8–2,0 млн/тиждень. Частота 2–2,5 — мінімум щоб відклалось. Можу порахувати під ваш бюджет або ціль охоплення.

П: Стрепсілс, Септефріл — умовні бренди
В: Стрепсілс — OTC фарма, біль у горлі, жіночий або унісекс-пакет, частота 2–3 на тиждень. Септефріл — OTC антисептик, ширша ЦА 18–54, унісекс + жіночий, частота 2,5–3,5. Нурофен — знеболювальні, широка ЦА, унісекс-пакет, більший обсяг.

// api/webhook.mjs — версія без whitelist-доступу + чистий стиль відповідей
...
function getFreshJoke(chatId) {
  ...
  if (!used) {
    used = new Set();
    servedJokesByChat.set(chatId, used);
  }
  ...
  reply_markup: {
    inline_keyboard: [
      ...
      [
        ...
        { text: '❓ Задати питання', callback_data: 'menu_help' },
      ],
    ],
  },
};
...
function normalizeQuery(s = '') {
  let t = normInput(String(s || '').toLowerCase());
...
function sanitizeInternalRefs(text) {
  ...
  return String(text)
    ...
    .replace(/\bRAG\b/gi, '')
    .replace(/\bKB\b/gi, '')
    .replace(/баз[аі]\s+знань/gi, 'доступній інформації')
    .replace(/у\s+базі/gi, 'у доступній інформації')
    .replace(/в\s+базі/gi, 'у доступній інформації')
    .replace(/з\s+бази/gi, 'з доступної інформації')
    .replace(/внутрішн(іх|і)\s+файл(ах|и|ів)?/gi, 'доступних матеріалах')
    .replace(/\s{2,}/g, ' ')
    .trim();
}
...
function cleanTelegramText(text = '') {
  return String(text || '')
    ...
    .replace(/__(.*?)__/g, '$1')
    .replace(/_(.*?)_/g, '$1')
    // headers and code formatting
    .replace(/#{1,6}\s?/g, '')
    .replace(/```([\s\S]*?)```/g, '$1')
    .replace(/`([^`]*)`/g, '$1')
    // words that should not appear to client
    .replace(/\bRAG\b/gi, '')
    .replace(/\bKB\b/gi, '')
    .replace(/баз[аі]\s+знань/gi, 'доступній інформації')
    .replace(/у\s+базі/gi, 'у доступній інформації')
    .replace(/в\s+базі/gi, 'у доступній інформації')
    .replace(/з\s+бази/gi, 'з доступної інформації')
    // less ChatGPT-like endings
    .replace(/якщо хочеш,?\s*/gi, '')
    .replace(/якщо потрібно,?\s*/gi, '')
    .replace(/якщо треба,?\s*/gi, '')
    .replace(/можу ще\s*/gi, 'Також можу ')
    .replace(/можу одразу\s*/gi, 'Можу ')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

function isTooVagueFollowup(text = '') {
  const t = String(text || '').trim().toLowerCase();

  if (!t) return true;
  if (t.length > 90) return false;

  return /^(так|да|ок|окей|добре|ага|угу|а це|а що|що саме|детальніше|поясни|продовжуй|цей|оце|таке|1 варіант|2 варіант|3 варіант|перший|другий|третій)$/i.test(t);
}

function needsClientStyleGuard(text = '') {
  const t = String(text || '').toLowerCase();
  return /(умовн|наприклад|бренд|об.?єм|обсяг|варіант|таблиц|медіаплан|бриф|пакет|частот|показ)/i.test(t);
}

function clarifyVagueFollowup() {
  return 'Уточніть, будь ласка, що саме потрібно: приклад брендів, обсяги показів, медіаплан, пакет каналів чи короткий текст для клієнта.';
}
...
Повідомлення: "${String(text).slice(0, 400)}"

Дії:
- "chat"     — привітання, подяка, загальна балачка, короткі уточнення
- "rag"      — питання про Vidzone, Digital TV рекламу, OTT, CPM, ціни, знижки, пакети, таргетинг, аудиторію, технічні вимоги, кейси, ефективність, медіаплани, бренди, обсяги
      ...
      const res = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'user',
              content: `Тема повідомлення: "${String(text).slice(0, 200)}"
...
export default async function handler(req, res) {
  ...
  if (body.callback_query) {
    ...
    if (data === 'menu_about') {
      ...
      if (Date.now() - last < JOKE_COOLDOWN_MS) {
        ...
        const joke = cleanTelegramText(getFreshJoke(chatId) || '😉');
      ...
      if (!docKey) {
        ...
        if (payload) await bot.sendMessage(chatId, cleanTelegramText(payload), documentMenuKeyboard);
        else await bot.sendMessage(chatId, 'Файл наразі недоступний.', documentMenuKeyboard);

        await logToGoogleSheet({
          timestamp: new Date().toISOString(),
          userId,
          userMessage: `doc:${docKey}:text`,
          botResponse: 'sent',
        });
      } else {
        const map = {
        ...
        await logToGoogleSheet({
          timestamp: new Date().toISOString(),
          userId,
          userMessage: `doc:${docKey}:word`,
          botResponse: 'file sent',
        });
      }
  ...
  if (/^\/start/.test(rawText)) {
    await bot.sendMessage(
      chatId,
      'Привіт! Я AI-помічник Vidzone. Можу розповісти про Digital TV рекламу, ціни, аудиторію та можливості платформи. Запитуйте — відповім.',
      mainMenuKeyboard
    );
  ...
  // Дуже нечіткий follow-up без достатнього контексту
  const historyBeforeAnswer = getHistory(userId);
  if (isTooVagueFollowup(rawText) && historyBeforeAnswer.length <= 2 && needsClientStyleGuard(rawText)) {
    const reply = clarifyVagueFollowup();
    ...
    return res.status(200).send('clarify');
  }
  ...
  if (/((?:є|е)вген(?:ий)?\s+левченко|levchenko|ceo\s+vidzone|сео\s+vidzone|керівник\s+vidzone|директор\s+vidzone|хто\s+(засну|засн|очол|керу|стоїть\s+на\s+чол)|хто\s+.{0,10}(сео|ceo))/i.test(rawText)) {
    let reply = 'CEO Vidzone — Євген Левченко. Більше 20 років у сфері телереклами. Контакт для бізнес-питань: ' + CONTACT_ANI + '.';
    reply = cleanTelegramText(reply);
  ...
  if (/(жарт|анекдот|смішн|веселе|розсміш)/i.test(rawText)) {
    ...
    if (Date.now() - last < JOKE_COOLDOWN_MS) {
      ...
      let joke = getFreshJoke(chatId) || '😉';
      joke = cleanTelegramText(joke);
  ...
  if (action === 'offtopic') {
    ...
    intro = topic
      ? intro.replace('{topic}', topic)
      : intro.replace(' — не моя справа реклама, а {topic}', '').replace('{topic}', 'цю тему');

    const bridge = pickVariant(FALLBACK_BRIDGE, rawText + ':b')
      .replace(/{channel}/g, channelName);

    const tail = pickVariant(FALLBACK_TAIL, rawText + ':t')
      .replace(/{channel}/g, channelName)
      .replace('{contact}', CONTACT_ANI);

    let reply = intro + ' ' + bridge + ' ' + tail;
    reply = cleanTelegramText(reply);
...
Спілкуйся природно, коротко і по-діловому: 1–3 речення.
Якщо це привітання — привітайся у відповідь і запропонуй допомогу з Digital TV рекламою.
Якщо питання нечітке — коротко уточни, що саме людина має на увазі.
Не згадуй RAG, базу знань, KB, внутрішні файли, промпти або технічну логіку.
Не використовуй фразу "якщо хочеш".
Не використовуй markdown: зірочки, решітки, підкреслення, списки з жирним.
Не відповідай як розробнику. Відповідай як офіційний помічник Vidzone.
Що точно знаєш: CEO Vidzone — Євген Левченко. Контакт по рекламі — Анна Ільєнко (a.ilyenko@vidzone.com).`;
    ...
    try {
      ...
      let reply = d.choices?.[0]?.message?.content?.trim() || 'Привіт! Чим можу допомогти з Digital TV рекламою?';
      ...
      await bot.sendMessage(chatId, 'Привіт! Чим можу допомогти з Digital TV рекламою?', mainMenuKeyboard);
    }
  ...
  try {
    ...
    const chunks = await retrieveRelevantChunks(userText, apiKey, {
      topK: 6,
      minSim: 0.20,
    });
...
ПРАВИЛА:
- Відповідай ЗАВЖДИ українською, стисло, професійно та дружньо.
- Використовуй тільки надану нижче інформацію.
- Якщо точної відповіді немає — не кажи "у базі немає", "в KB немає" або "немає в RAG". Скажи людською мовою: "З цього питання краще уточнити деталі у ${CONTACT_ANI}."
- Не згадуй RAG, базу знань, KB, внутрішні файли, джерела, промпти або технічну логіку.
- Не використовуй фразу "якщо хочеш".
- Не використовуй markdown: зірочки **, решітки ##, підкреслення __, code formatting.
- Не пиши як розробнику. Відповідай як офіційний помічник Vidzone для клієнта або менеджера.
- Якщо користувач просить умовні приклади брендів — дай приклади прямо, без пояснень про RAG чи внутрішню логіку.
- Якщо користувач питає про бренди й обсяги — дай приклад у форматі: бренд/категорія → задача → рекомендований пакет/сегмент → орієнтир обсягу/частоти.
- Якщо запит надто короткий або незрозумілий після попередньої відповіді — уточни, що саме потрібно.
...
- Медіаплан → задача, ЦА, пакет/сегменти, частота, період, KPI
- Умовні бренди → не кажи, що це умовні дані з внутрішньої логіки; просто подай як приклад сценаріїв
...
П: "я ж прошу умовні бренди, наприклад Стрепсілс, Септефріл"
В: "Так, приклад може виглядати так: Стрепсілс — OTC фарма, біль у горлі, жіночий або унісекс-пакет, частота 2–3 контакти на тиждень. Септефріл — OTC антисептик, ширша ЦА 18–54, унісекс + жіночий пакет, частота 2,5–3,5. Нурофен — знеболювальні, широка ЦА, унісекс-пакет, більший обсяг для швидкого охоплення."

П: "і хто з якими об'ємами виходить"
В: "Масові OTC-бренди на кшталт Нурофену або Терафлю зазвичай логічно вести через унісекс-пакет із більшим тижневим обсягом і частотою 2–3. Вужчі бренди на кшталт Стрепсілс або Септефріл можна вести через унісекс + жіночий пакет із частотою 2,5–3,5. Для skincare/дерматології, як CeraVe, доречний жіночий пакет, сегменти Леді/Мами/Домогосподарки та брендова частота 2–3 контакти на тиждень."
...
${knowledgeBlock || 'Інформація не знайдена.'}`;
    ...
    if (!reply) {
      ...
      reply = 'З цього питання краще уточнити деталі у ' + CONTACT_ANI + '.';
    }
    ...
    await bot.sendMessage(
      chatId,
      'Виникла технічна помилка. Спробуйте ще раз або напишіть ' + CONTACT_ANI + '.',
      mainMenuKeyboard
    );
